...
LeveltrackerImport()
{
	...
	gemtranslations := LLK_FileRead("data\leveling tracker\gemtranslations.json", 0, "UTF-8")
	gemtranslations := Json.Load(gemtranslations)
	...
	For act_index, act in import ;parse all acts
	{
		For step_index, step in act.steps ;parse steps in nth act
		{
			...
			If (step.type = "gem_step")
			{
				...
				gemName := gems[gemID].name
				gemNameCN := gemtranslations.HasKey(gemName) ? gemtranslations[gemName].name : gemName
				...
				; step_text .= (step.rewardType = "vendor" ? "buy gem: " : "take reward: ") . (color ? "(color:"color ")" : "") StrReplace(gemNameCN, " ", "_")

				step_text .= (step.rewardType = "vendor" ? "buy gem: " : "take reward: ") . (color ? "(color:"color ")" : "") StrReplace(gemName, " ", "_") . (gemNameCN ? "(" gemNameCN ")" : "")
